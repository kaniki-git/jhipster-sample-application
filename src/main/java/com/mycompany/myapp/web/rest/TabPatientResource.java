...
public class TabPatientResource {
    ...
    public List<TabPatient> getAllTabPatients(@RequestParam(required = false) String filter) {
        if ("tabpersonnel-is-null".equals(filter)) {
            log.debug("REST request to get all TabPatients where tabPersonnel is null");
            return StreamSupport
                .stream(tabPatientRepository.findAll().spliterator(), false)
                .filter(tabPatient -> tabPatient.getTabPersonnel() == null)
